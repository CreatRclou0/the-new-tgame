...
export class TrafficLightController {
    constructor() {
        ...
        // Fixed mode state - completely independent
        this.fixedState = {
            currentPhase: 0, // 0: WE green, 1: WE yellow, 2: WE red, 3: NS green, 4: NS yellow, 5: NS red
            phaseTimer: 0,
            isActive: false
        };
    ...
    initializeFixedMode() {
        ...
        this.fixedState = {
            currentPhase: 0, // Start with West-East green
            phaseTimer: 0,
            isActive: true
        };
    ...
    updateFixedMode(deltaTime) {
        ...
        switch (this.fixedState.currentPhase) {
            ...
            case 2: // West-East Red
                if (this.fixedState.phaseTimer >= this.settings.RED_DURATION) {
                    this.advanceFixedPhase();
                }
                break;
            case 3: // North-South Green
                if (this.fixedState.phaseTimer >= this.settings.GREEN_DURATION) {
                    this.advanceFixedPhase();
                }
                break;
            case 4: // North-South Yellow
                if (this.fixedState.phaseTimer >= this.settings.YELLOW_DURATION) {
                    this.advanceFixedPhase();
                }
                break;
            case 5: // North-South Red
                if (this.fixedState.phaseTimer >= this.settings.RED_DURATION) {
                    this.advanceFixedPhase();
                }
                break;
